    ...
    public class GameServiceLocator : MonoBehaviour
    {
        ...
        public static GameServiceLocator Instance
        {
            get
            {
                if (!_instance)
                {
                    Debug.LogWarningFormat("Accesing {0} before it's Awake phase", typeof(PlayerPrefsAdapter).Name);
                }
                return _instance;
            }
        }
        ...
        private void Awake()
        {
            ...
            {
                Debug.LogWarningFormat("Make sure there is one {0} in the scene", typeof(PlayerPrefsAdapter).Name);
                Destroy(this);
                return;
            }
            else
